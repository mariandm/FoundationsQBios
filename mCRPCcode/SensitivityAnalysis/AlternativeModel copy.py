...
allSolution=np.array(allSolution)
rara=np.random.poisson(allSolution[:,3])

k1=[1.5, 10000, 10000]
k2=[0.5, 100, 10000]
k3=[15000,10000,10000]

percentages=np.arange(0.9,1.1,0.005)
#Parameter order=sigmaPSA, r[0], r[1], r[2], k1[0], k1[1], k1[2], k2[0], k2[1], k2[2]
parameters=[sigmaPSA,r[0],r[1],r[2],k1[0], k1[1], k1[2], k2[0], k2[1], k2[2], k3[0], k3[1], k3[2]]

LH_vector=np.zeros([len(parameters),len(percentages)])

for curr_param in range(len(parameters)):
    
    for curr_percentage in range(len(percentages)):
        
        tmp_params=copy.deepcopy(parameters)
        
        tmp_params[curr_param]=tmp_params[curr_param]*percentages[curr_percentage]
        
        #Asignar parametros de tmp_params
        sigmaPSA=tmp_params[0]
        r[0]=tmp_params[1]  
        r[1]=tmp_params[2]  
        r[2]=tmp_params[3] 
        k1[0]=tmp_params[4]  
        ...
        #Al final del desmadre
        tmp_pos=np.log(poisson.pmf(rara,allSolution[:,3]))
        tmp_pos[np.isneginf(tmp_pos)]=0
        LH_vector[curr_param,curr_percentage]=np.cumsum(tmp_pos)[-1]
        


for i in range(len(LH_vector)):
    plt.plot(percentages,LH_vector[i])
plt.legend(["sigmaPSA", "r[0]", "r[1]", "r[2]", "k1[0]", "k1[1]", "k1[2]", "k2[0]", "k2[1]", "k2[2]"])
