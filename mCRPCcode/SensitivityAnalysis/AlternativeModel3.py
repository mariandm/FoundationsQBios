...
percentages=np.arange(0.9,1.1,.001)

for curr_matrix in [6]:
    ...
    finalDensity=np.zeros(len(percentages))
    TminusProportion=np.zeros(len(percentages))
    TminusColonizationTime=np.zeros(len(percentages))
    ...
    for percent in range(len(percentages)):
        ...
        finalDensity[percent]=np.sum(y[0:3])
        TminusProportion[percent]=y[2]/finalDensity[percent]
        print(TminusProportion[percent])
    ...
    print(TminusProportion-TminusProportion[percentages==1])
    plt.plot(percentages,-np.abs(TminusProportion-TminusProportion[percentages==1]))
    plt.xlabel("K*/K")
